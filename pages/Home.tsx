...
import { MOCK_CITATIONS } from '../constants';
...
const Home: React.FC<HomeProps> = ({ allBooks, allPeople }) => {
  ...
  const mostCitedBooks = useMemo(() => {
    const citationCounts = MOCK_CITATIONS.reduce((acc, cit) => {
  ...
  const featuredCuratorBooks = MOCK_CITATIONS.filter(c => c.personId === featuredCurator?.id)
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-6">
        {/* Hero Section */}
        {featuredHeroPerson && (
          <section className="relative text-white py-20 md:py-32 rounded-lg overflow-hidden my-12 bg-black">
              <div 
        ...
        <section className="mb-20">
          <div className="flex items-center justify-between mb-8 border-t border-[var(--border-color)] pt-12">
            ...
            <Link to="/latest" className="font-semibold text-sm hover:text-black">Ver todos</Link>
          </div>
        ...
        <section className="mb-20">
          <div className="flex items-center justify-between mb-8 border-t border-[var(--border-color)] pt-12">
            ...
            <Link to="/most-cited" className="font-semibold text-sm hover:text-black">Ver todos</Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-x-8 gap-y-16">
