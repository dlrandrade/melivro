...
import { MOCK_ACTIVITIES, MOCK_BOOKS, MOCK_PEOPLE } from '../constants';
import { Activity, BookStatus, ActivityType } from '../types';
...
const Feed: React.FC = () => {
  return (
    <>
      <MetaTags 
        title="Feed da Comunidade | livr.me"
        description="Veja o que os outros estão lendo e definindo como meta na comunidade livr.me."
      />
      <div className="max-w-3xl mx-auto px-6 py-12">
        <div className="text-center mb-12">
          <h1 className="font-serif text-5xl font-bold text-black tracking-tighter">Feed da Comunidade</h1>
          <p className="text-lg text-gray-500 mt-2">Veja o que os outros estão lendo e definindo como meta.</p>
        </div>
        <div className="space-y-8">
          {MOCK_ACTIVITIES.map(act => <ActivityCard key={act.id} activity={act} />)}
        </div>
      </div>
    </>
  );
