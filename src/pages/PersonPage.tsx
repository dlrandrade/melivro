...
const PersonPage: React.FC<PersonPageProps> = ({ allPeople, allBooks, allCitations }) => {
  ...
  if (!person) return (
    <>
      <MetaTags title="Pessoa não encontrada | MeLivro" description="A pessoa que você está procurando não foi encontrada." />
  ...
  return (
    <>
      <MetaTags
        title={`${person.name} | Recomendações de Livros no MeLivro`}
